...
const StepForm = () => {
  ...
  const validateAllSteps = () => {
    ...
    for (let stepNum = 1; stepNum <= 3; stepNum++) {
      ...
      // 🧼 Removed stepNum === 2 block that required woundReferenceFile
  ...
  const submitStep1 = async () => {
    ...
    try {
      ...
      const patientId = data.id || data.patientId || data.data?.id;
      if (!patientId) {
        console.error("Unexpected API response format:", data);
        throw new Error("Server didn't return a patient ID. Please check the response format.");
      }
  ...
  const submitStep3 = async (id) => {
    try {
      ...
      const response = await fetch(`https://webstrategy.co.in/rssdi/api/step3/${id}`, {
        method: "POST",
        body: formDataToSubmit,
      });
  ...
  const calculateFollowUpDate = (currentDate) => {
    ...
    followUpDate.setMonth(followUpDate.getMonth() + 6); // Add 6 months
    return followUpDate;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Prevent duplicate Step 1 submission
      if (!patientIdToUse && !step1SubmittedRef.current) {
        step1SubmittedRef.current = true; // Mark Step 1 as submitted
        patientIdToUse = await submitStep1();
        setPatientId(patientIdToUse);
      }

      // ✅ Proceed with Step 2 and 3 submissions
  ...
  const handleSaveAndContinue = async () => {
    ...
    try {
      ...
      // Use newPatientId instead of stale state
      if (step !== 1 || newPatientId) {
        setStep(prev => Math.min(prev + 1, 3));
        window.scrollTo({ top: 0, behavior: "smooth" });
        toast.success("Progress saved successfully!");
